...
plot_algo_dual <- function(df, farrington, cusum, year_target, 
                           ...
                           disease_label = "Số ca bệnh") { # Tham số để đổi tên bệnh
  ...
  p <- ggplot() + 
    # Phần cột: hiển thị tên bệnh trực tiếp vào legend
    geom_col(data = df_target, aes(x = week, y = cases, fill = disease_label),
             color = "black", width = 1, alpha = 0.4) +
    
    # Phần đường và ngưỡng
    geom_line(data = df_target, aes(x = week, y = farrington, color = "Farrington"), size = 1) +
    geom_line(data = df_target, aes(x = week, y = cdc, color = "Mean+2SD"), size = 1) +
    geom_hline(aes(yintercept = seasonal, color = "Ngưỡng mùa"), 
               linetype = "dashed", size = 1) +
    
    # Phần cảnh báo (Outbreak)
    ...
    geom_point(data = filter(df_target, outbreak_cdc == 1),
               aes(x = week, y = cases, color = "Mean+2SD"),
               size = 3, shape = 17, stroke = 1.2) +
    ...
    # Xử lý Màu sắc (Color) - Bỏ Title
    scale_color_manual(
      name = NULL, 
      values = c("Farrington" = "#E41A1C", "CUSUM" = "orange", 
                 "Mean+2SD" = "#4DAF4A", "Ngưỡng mùa" = "black")
    ) +
    # Xử lý Màu lấp đầy (Fill) - Bỏ Title
    scale_fill_manual(
      name = NULL, 
      values = setNames("steelblue", disease_label)
    ) +
